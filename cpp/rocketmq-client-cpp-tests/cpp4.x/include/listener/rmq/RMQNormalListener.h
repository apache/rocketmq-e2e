...
class RMQNormalListener : public MQCollector<std::string>, public rocketmq::MessageListenerConcurrently {
private:
    ...
    virtual rocketmq::ConsumeStatus consumeMessage(const std::vector<rocketmq::MQMessageExt>& msgs) override {
        ...
        for (const auto& msg : msgs) {
            if (reconsumeTimes == 0 || reconsumeTimes == msg.getReconsumeTimes()-1) {
                // rocketmq::MQMessage message(msg.getTopic(),msg.getTags(),msg.getKeys(),msg.getBody());
                dequeueMessages->addData(msg.getMsgId());
...
//         for (const auto& msg : msgs) {
//             if (reconsumeTimes == 0 || reconsumeTimes == msg.getReconsumeTimes()-1) {
//                 dequeueMessages->addData(msg.getBody());
//                 result = consumeStatus;
//             }else{
//                 // if (isEvenNumber) {
//                 //     int body = atoi(message.getBody().c_str());
